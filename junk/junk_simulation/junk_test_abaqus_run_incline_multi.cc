...
double time_bie;
int main() {
    //std::string filename = "abaqus_cpp_two_faults_5degree_0d_flat.inp";
   // std::string filename = "abaqus_Rice_fish_bone_8_25m.inp";
    std::string filename = "abaqus_cpp_three_fault.inp";
   ...
   // std::cout<<x<<std::endl;
    ...
    {
        if ((x_main(i)<=(x_main.minCoeff()+x_main.maxCoeff())/2+0.6e3)&&(x_main(i)>=(x_main.minCoeff()+x_main.maxCoeff())/2-0.6e3))
        {
            T_0[0](2*i) = 31.0e6;
        }
    }
   ...
   // file.write((char*)(u_n.data()),u_n.size()*sizeof(double));
    ...
    for (int j=0;j<numt;j++)
    {
        ...
        for (int i=0 ; i<num_faults; i++)
        {
            ...
            Slip_Weakening_lumpM(M_global_vec, Fault_surf_index[2*i], Fault_surf_index[2*i+1], fe_global, dt, dx, dy, nx_faults[i]-1, delt_v_n[i], delt_u_n[i], T_0[i], tau_s[i], mu_s, mu_d, Dc, Ndofn, M_pos, M_neg , F_fault, T_c[i], fault_angle[i]);
