...
//    int num_faults = fault_pos.rows();
    ...
    for (int j=0;j<numt;j++)
    {
        ...
        for (int i=0 ; i<num_faults; i++)
        {
            ...
            Slip_Weakening_reg(M_global_vec, Fault_surf_index[2*i], Fault_surf_index[2*i+1], fe_global, dt, dx, dy, nx_faults[i]-1, delt_v_n[i], delt_u_n[i], T_0[i], tau_s[i], mu_s, mu_d, Dc, Ndofn, M, F_fault, T_c[i]);
