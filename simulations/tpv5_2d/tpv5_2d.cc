...
int main() {
  ...
  // std::string filename = "tpv5_2d_50km_2km_100m.inp";
  //  std::string filename = "tpv5_2d_50km_4km_100m.inp";

  //  std::string filename = "tpv5_2d_50km_100km_200m.inp";
    std::string filename = "tpv5_2d_50km_4km_100m_shfitAB.inp";
   ...
   // double fault_angle  = 0/180.0*M_PI;
    ...
    mesh_Abaqus_multi_faults_withAB(filename, Node, Element, BIE_top, BIE_bot,AB_left,AB_right,fault_nodes,num_faults);
    ...
    for (int i=0; i<num_faults; i++)
    {
        ...
        std::cout<<"angel="<<fault_angle[i]/3.14*180<<std::endl;
        std::cout<<"abs(sigma_f[i])*0.6="<<std::abs(sigma_f[i])*0.6<<"tau_f[0]="<<tau_f[i]<<std::endl;
   ...
   // std::cout<<x<<std::endl;
    ...
    for (int j=0; j<num_faults;j++)
    {
      ...
      //      T_0[j](2*i+1) = sigma_f[j];
       //     T_0[j](2*i) =  tau_f[j];
        ...
        if (j==0)
        {
            mu_s[j] =0.677*ArrayXd::Ones(nx_faults[j], 1);
        }
        else
        {
            mu_s[j] =0.8*ArrayXd::Ones(nx_faults[j], 1);
        }
    }
    ...
    {
        if ((x_main(k)<=(0.0+1.5e3))&&(x_main(k)>=(0.0-1.5e3)))
        {
            T_0[0](2*k) = 81.6e6;
        }
        else if ((x_main(k)<=(-7.5e3+1.5e3))&&(x_main(k)>=(-7.5e3-1.5e3)))
        {
            T_0[0](2*k) = 78.0e6;
        }
        else if ((x_main(k)<=(7.5e3+1.5e3))&&(x_main(k)>=(7.5e3-1.5e3)))
  ...
  //  VectorXi BIE_top_surf_index = VectorXi::Zero(Ndofn*(BIE_top_surf_nodes.size()),1);
   // VectorXi BIE_bot_surf_index = VectorXi::Zero(Ndofn*(BIE_bot_surf_nodes.size()),1);
    ...
    std::cout<<AB_left[0]<<std::endl;
    std::cout<<AB_left[1]<<std::endl;
    ...
    // Calculating T_abc for absorbing bc
    MatrixXd coord = MatrixXd::Zero(2,2);
    VectorXi Element_0= Element.row(0);
    coord.row(0) = Node.row(AB_left[0]);
    coord.row(1) = Node.row(AB_left[1]);
    MatrixXd T_abc;
    cal_T_abc(coord, density, T_abc,v_s,v_p);
    std::cout<<T_abc<<std::endl;
