...
//    int num_faults = fault_pos.rows();
    ...
    std::cout<<BIE_top_surf_nodes<<std::endl;
    ...
    std::string  x_fault= "results/x_fault_"+std::to_string(0)+".txt";
    ...
    for (int j=0;j<numt;j++)
    {
        ...
        for (int i=0 ; i<num_faults; i++)
        {
            ...
            Slip_Weakening_bimat(M_global_vec, Fault_surf_index[2*i], Fault_surf_index[2*i+1], fe_global, dt, dx, dy, nx_faults[i]-1, delt_v_n[i], delt_u_n[i], T_0[i], tau_s[i], mu_s[i], mu_d, Dc, Ndofn, M, F_fault, T_c[i]);
