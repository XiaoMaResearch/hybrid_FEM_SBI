...
#include "Slip_Weakening_reg_bimat.hpp"
...
//    int num_faults = fault_pos.rows();
    ...
    // Second material
    double density_2 = 2670.0;
    double v_s_2 = 2887.0;
    double v_p_2 = 5000.0;
    double G_2= pow(v_s_2,2)*density_2;
    double Lambda_2 = pow(v_p_2,2)*density_2-2.0*G_2;
    double E_2  = G_2*(3.0*Lambda_2+2.0*G_2)/(Lambda_2+G_2);
    double nu_2 = Lambda_2/(2.0*(Lambda_2+G_2));
    // Time
    double alpha = 0.4;
    ...
    for (int i=0;i<num_faults;i++)
    {
        ...
        tau_s[i] = 0.677*ArrayXd::Ones((nx_faults[i]),1)*120.0e6;
    }
    ...
    for (int j=0; j<num_faults;j++)
    {
        ...
        if (j==0)
        {
            ...
            for (int i=0 ; i<nx_faults[j]; i++)
            {
                if ((x(i)<=(fault_pos(j,1)+fault_pos(j,2))/2+1.5e3)&&(x(i)>=(fault_pos(j,1)+fault_pos(j,2))/2-1.5e3))
                {
                    T_0[j](2*i) = 81.6e6;
                    T_c[j](2*i) =  T_0[j](2*i);
                }
    ...
    ArrayXd mu_s = ArrayXd::Zero(nx_faults[0], 1);
    VectorXd x = VectorXd::LinSpaced(nx_faults[0], fault_pos(0,1), fault_pos(0,2));
    for (int i=0 ; i<nx_faults[0]; i++)
    {
        if ((x(i)<=(fault_pos(0,1)+fault_pos(0,2))/2+15.0e3)&&(x(i)>=(fault_pos(0,1)+fault_pos(0,2))/2-15.0e3))
        {
            mu_s(i) = 0.677;
        }
        else
    ...
    MatrixXd ke_2 = MatrixXd::Zero(8,8);
    ...
    cal_ke (coord,E_2,nu_2,ke_2);

    // BIE part initiation
    // Setting up the material property for the BIE code
    Material BIE_top_mat = Material(E_2,nu_2,density);
    ...
    for (int j=0;j<numt;j++)
    {
        ...
        cal_fe_global_const_ke_bimat(Node,Element,n_nodes, n_el, index_store, q, u_n, v_n, Ndofn, ke, ke_2, fe_global);
        // Friction subroutine
        VectorXd F_total = F_ext_global-fe_global;
        for (int i=0 ; i<num_faults; i++)
        {
            VectorXd F_fault = VectorXd::Zero(Ndofn*(nx_faults[i]),1);
            Slip_Weakening_reg_bimat(M_global_vec, Fault_surf_index[2*i], Fault_surf_index[2*i+1], fe_global, dt, dx, dy, nx_faults[i]-1, delt_v_n[i], delt_u_n[i], T_0[i], tau_s[i], mu_s, mu_d, Dc, Ndofn, M, F_fault, T_c[i]);
